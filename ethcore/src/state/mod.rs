...
impl State {
	...
	/// Adds `by` to the balance of account `to`.
	///
	/// If `no_empty`, will not create an empty account.
	///
	/// If `kill_empty`, will remove the destination account if it would otherwise be empty.
	pub fn add_balance_and_cleanup(&mut self, to: &Address, by: &U256, no_empty: bool, kill_empty: bool) {
		if !no_empty || !by.is_zero() {
			self.add_balance(to, by);
		} else if kill_empty /*&& by.is_zero()*/ && self.ensure_cached(to, RequireCache::CodeSize, |a| a.map(Account::is_basically_empty).unwrap_or(false)) {
